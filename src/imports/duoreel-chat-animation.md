...
Each bubble `0.5s` apart. The `twoHoursLater` image gets an extra `1s` gap before it (feels like time passing). Total sequence ends at `~5.5s`.

| Item | Delay |
|------|-------|
| "Want to watch a movie tonight? 🍿" | `0.3s` |
| "Sure! How about Barbie?" | `0.8s` |
| "Already saw it twice! Oppenheimer?" | `1.3s` |
| "It's 3 hours and I have work tomorrow 😅" | `1.8s` |
| "The Notebook? John Wick? Inception?" | `2.3s` |
| "Too sappy / a puppy dies 😭 / too confusing 🤯" | `2.8s` |
| Two Hours Later image | `3.8s` (extra 1s gap) |
| "Did you find something? 😴" | `4.3s` |
| "Ugh this is impossible 😩" | `4.8s` |
| Result row | `5.3s` |

### With DuoReel — delayed start

The With DuoReel chat starts after the Without card finishes: `5.3s + 0.6s (transition duration) = ~5.9s` after the section enters the viewport.

| Item | Delay |
|------|-------|
| "Wait for it…" placeholder | visible immediately (0s), hidden at `5.9s` |
| "Want to watch a movie tonight? 🍿" | `6.1s` |
| "Let's check our DuoReel! 🎬" | `6.6s` |
| Grand Budapest cards | `7.1s` |
| "Perfect! I've been wanting to watch that! 😍" | `7.6s` |
| "Same! Starting it now 🍿" | `8.1s` |
| Result row | `8.6s` |
  ...
  <div className="grid md:grid-cols-2 gap-8 items-stretch">
    ...
    <div className="rounded-2xl border border-green-500/20 bg-gradient-to-br from-green-500/8 via-slate-900/50 to-slate-900/50 p-8 flex flex-col">
      ...
      <div className="flex flex-col gap-3 flex-1 relative">
        ...
        >
          <div className="flex gap-1.5 items-center text-slate-400 text-sm font-medium animate-pulse">
...
- **The "wait for it…" placeholder** uses `opacity: 0` with `transition: opacity 0.4s ease-out 5.9s` — it fades out exactly when the Without card finishes. It's `position: absolute` so it doesn't affect layout height.
...
- [ ] On scroll into view: Without card bubbles appear one by one, ~0.5s apart
- [ ] "Two Hours Later" image appears with an extra ~1s pause before it (feels like time passing)
- [ ] Without result row appears last at ~5.3s
- [ ] With DuoReel card border and label are visible immediately
- [ ] "Wait for it…" pulsing dots + italic text visible in With card while Without plays
- [ ] At ~5.9s: "Wait for it…" fades out
- [ ] With DuoReel chat bubbles begin at ~6.1s and appear every ~0.5s
- [ ] Grand Budapest cards appear as a single unit at ~7.1s
- [ ] With result row appears last at ~8.6s
