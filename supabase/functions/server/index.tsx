...
async function createNotification(
  ...
  const current =
    (await kv.get(`notifications:unread:${userId}`)) || 0;
  await kv.set(
  ...
  console.log(
    `Created ${type} notification for user ${userId}`,
  );
}
...
async function pMap<T, R>(
  ...
  const results: PromiseSettledResult<R>[] = new Array(
    items.length,
  );
  let nextIndex = 0;
...
app.post("/make-server-5623fde1/auth/signup", async (c) => {
  try {
    ...
    await kv.set(`user:search:${email}`, {
      userId: data.user.id,
      name,
      email,
    });
...
app.post(
  "/make-server-5623fde1/api/ensure-profile",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      if (authError || !user?.id) {
        return c.json(
          {
            error: `Unauthorized - Invalid token: ${authError?.message}`,
          },
          401,
        );
      }
      ...
      console.log(
        `ensure-profile: created new profile for ${email} (${user.id})`,
      );
      return c.json({ exists: false, profile, created: true });
    } catch (error) {
      console.error("ensure-profile error:", error);
      return c.json(
        { error: `Failed to ensure profile: ${error}` },
        500,
      );
    }
  },
...
app.post("/make-server-5623fde1/profile", async (c) => {
  try {
    ...
    const updatedProfile = {
      ...
      ...(letterboxdUsername !== undefined && {
        letterboxdUsername,
      }),
      ...(letterboxdLastSyncGuid !== undefined && {
        letterboxdLastSyncGuid,
      }),
    };
...
app.post("/make-server-5623fde1/partner/connect", async (c) => {
  try {
    ...
    const partner = partnerData?.users?.find(
    ...
    const existingRequest = await kv.get(
      `partner_request:${partner.id}:${user.id}`,
    );
    if (existingRequest) {
    ...
    return c.json(
      { error: `Failed to send partner request: ${error}` },
      500,
    );
  }
});
...
app.get(
  "/make-server-5623fde1/partner/requests/incoming",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
...
app.get(
  ...
  async (c) => {
    try {
      ...
      const allRequests =
        await kv.getByPrefix(`partner_request:`);
      const outgoingRequests = allRequests.filter(
        (req: any) => req.fromUserId === user.id,
      );
...
app.post("/make-server-5623fde1/partner/accept", async (c) => {
  try {
    ...
    const request = await kv.get(
      `partner_request:${user.id}:${fromUserId}`,
    );
    if (!request) {
    ...
    const partnerProfile =
      (await kv.get(`user:${fromUserId}`)) || {};

    await kv.set(`user:${user.id}`, {
      ...userProfile,
      partnerId: fromUserId,
    });
    await kv.set(`user:${fromUserId}`, {
      ...partnerProfile,
      partnerId: user.id,
    });
    ...
    return c.json(
      { error: `Failed to accept request: ${error}` },
      500,
    );
  }
});
...
app.post("/make-server-5623fde1/partner/reject", async (c) => {
  try {
    ...
    return c.json(
      { error: `Failed to reject request: ${error}` },
      500,
    );
  }
});
...
app.post("/make-server-5623fde1/partner/remove", async (c) => {
  try {
    ...
    return c.json(
      { error: `Failed to remove partner: ${error}` },
      500,
    );
  }
});
...
app.get(
  "/make-server-5623fde1/partner/invite-code",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      return c.json(
        { error: `Failed to get invite code: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/partner/accept-invite",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      const existingRequest = await kv.get(
        `partner_request:${user.id}:${inviterId}`,
      );
      if (existingRequest) {
        return c.json({
          success: true,
...
app.post(
  "/make-server-5623fde1/partner/regenerate-invite",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      console.log(
        `Regenerated invite code for user ${user.id}: ${code}`,
      );
      ...
      return c.json(
        { error: `Failed to regenerate invite code: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/letterboxd/validate",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
  ...
  try {
    ...
    try {
      ...
      if (!letterboxdUsername) {
        return c.json(
          { error: "No Letterboxd username set" },
          400,
        );
      }
      ...
      if (rssResponse.status === 429) {
        return c.json(
          {
            error:
              "Letterboxd rate limit exceeded. Try again in a few minutes.",
          },
          429,
        );
      }
      ...
      if (!rssResponse.ok) {
        return c.json(
          {
            error: `Failed to fetch Letterboxd RSS (HTTP ${rssResponse.status})`,
          },
          502,
        );
      }
      ...
      while ((match = itemRegex.exec(rssText)) !== null) {
        ...
        const movieIdMatch = itemXml.match(
        ...
        const watchedDateMatch = itemXml.match(
          /<letterboxd:watchedDate>([\s\S]*?)<\/letterboxd:watchedDate>/,
        );
      ...
      if (items.length === 0) {
        return c.json({
          synced: 0,
          message: "No movie entries found in RSS feed",
        });
      }

      // Find new entries since last sync
      const lastSyncGuid = profile?.letterboxdLastSyncGuid;
      const lastSyncIndex = lastSyncGuid
        ? items.findIndex((i) => i.guid === lastSyncGuid)
        : -1;
      // Items at index 0 are newest; stop before the last-synced item
      const newItems =
        lastSyncIndex === -1
          ? items
          : items.slice(0, lastSyncIndex);

      if (newItems.length === 0) {
        return c.json({
          synced: 0,
          message: "No new entries since last sync",
        });
      }
          ...
          batch.map(async (item) => {
            try {
              ...
              const existing = await kv.get(
                `watched:${user.id}:${item.tmdbMovieId}`,
              );
              if (existing) {
                ...
                await kv.set(
                  ...
                  {
                    ...existing,
                    // Always ensure id is present — older entries may have been stored
                    // without it (e.g. early Letterboxd sync path), which causes
                    // interactions/all to skip the entry on the next load.
                    id: item.tmdbMovieId,
                    rating: item.rating,
              ...
              const sanitizedMovie = {
                ...
                genre_ids: (tmdbMovie.genres || []).map(
                  (g: any) => g.id,
                ),
              ...
              synced++;
              syncedTitles.push(
                tmdbMovie.title || `Movie ${item.tmdbMovieId}`,
              );
              syncedMovieIds.push(item.tmdbMovieId);
            } catch (err) {
              console.error(
                `Failed to sync TMDB movie ${item.tmdbMovieId}:`,
                err,
              );
              failed.push(item.tmdbMovieId);
            }
      ...
      console.log(
        `Letterboxd sync for user ${user.id}: ${synced} synced, ${failed.length} failed`,
      );
      return c.json({
        synced,
...
app.get(
  "/make-server-5623fde1/notifications/matches",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      const userLikes = await getByPrefixPaginated(
        `like:${user.id}:`,
      );
      ...
      const partnerLikes = await getByPrefixPaginated(
        `like:${profile.partnerId}:`,
      );
      const partnerLikeIds = new Set(
        partnerLikes.map((like: any) => like.movieId),
      );
      ...
      const newMatches = userLikes.filter((like: any) => {
        ...
        const partnerLike = partnerLikes.find(
          (pl: any) => pl.movieId === like.movieId,
        );
        const partnerLikeTimestamp =
          partnerLike?.timestamp || 0;

        return (
          Math.max(likeTimestamp, partnerLikeTimestamp) >
          lastSeen
        );
      });
      ...
      console.error(
        "Error getting match notifications:",
        error,
      );
      return c.json(
...
app.post(
  "/make-server-5623fde1/notifications/matches/seen",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      return c.json(
        { error: `Failed to mark as seen: ${error}` },
        500,
      );
    }
  },
...
app.get("/make-server-5623fde1/notifications", async (c) => {
  try {
    ...
    const notifications = await kv.getByPrefix(
...
app.get(
  "/make-server-5623fde1/notifications/unread-count",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      const count =
        (await kv.get(`notifications:unread:${user.id}`)) || 0;
      return c.json({
      ...
      return c.json(
        { error: `Failed to fetch unread count: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/notifications/mark-read",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      const notification = await kv.get(
        `notification:${user.id}:${notificationId}`,
      );
      if (!notification) {
      ...
      if (!notification.read) {
        ...
        const current =
          (await kv.get(`notifications:unread:${user.id}`)) ||
          0;
      ...
      return c.json(
        { error: `Failed to mark as read: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/notifications/mark-all-read",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      return c.json(
        { error: `Failed to mark all as read: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/notifications/clear-all",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      return c.json(
        { error: `Failed to clear notifications: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/notifications/import-complete",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      const { type, imported, failed, total } =
        await c.req.json();
      ...
      console.error(
        "Error creating import notification:",
        error,
      );
      return c.json(
...
app.get("/make-server-5623fde1/partner", async (c) => {
  try {
    ...
    const partnerProfile = await kv.get(
      `user:${profile.partnerId}`,
    );
    const { data: partnerAuth } =
      await supabase.auth.admin.getUserById(profile.partnerId);
    ...
    return c.json(
      { error: `Failed to fetch partner: ${error}` },
      500,
    );
  }
});
...
app.post("/make-server-5623fde1/movies/like", async (c) => {
  try {
    ...
    await kv.set(`liked:${user.id}:${movie.id}`, {
      ...movie,
      timestamp,
    });
    await kv.set(`like:${user.id}:${movie.id}`, {
      movieId: movie.id,
      timestamp,
    });
    ...
    if (profile?.partnerId) {
      const partnerLike = await kv.get(
        `liked:${profile.partnerId}:${movie.id}`,
      );
      if (partnerLike) {
        isMatch = true;
        await kv.set(`match:${user.id}:${movie.id}`, {
          ...movie,
          timestamp,
        });
        await kv.set(`match:${profile.partnerId}:${movie.id}`, {
          ...movie,
          timestamp,
        });
        ...
        const partnerProfile =
          (await kv.get(`user:${profile.partnerId}`)) || {};
        ...
        const allMatches = await getByPrefixPaginated(
          `match:${user.id}:`,
        );
        ...
        if (milestones.includes(matchCount)) {
          console.log(
            `🎉 Milestone reached: ${matchCount} matches for user ${user.id}!`,
          );
          ...
          await createNotification(
            ...
            "match_milestone",
            {
              fromUserId: user.id,
              fromName:
                currentUserProfile.name || "Your partner",
              milestoneCount: matchCount,
            },
          );
        }
    ...
    return c.json(
      { error: `Failed to like movie: ${error}` },
      500,
    );
  }
});
...
// Unlike a movie
app.delete(
  "/make-server-5623fde1/movies/like/:movieId",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      return c.json(
        { error: `Failed to unlike movie: ${error}` },
        500,
      );
    }
  },
...
app.post("/make-server-5623fde1/movies/dislike", async (c) => {
  try {
    ...
    await kv.set(`disliked:${user.id}:${movieId}`, {
      movieId,
      timestamp: Date.now(),
    });
    ...
    return c.json(
      { error: `Failed to dislike movie: ${error}` },
      500,
    );
  }
});
...
app.get("/make-server-5623fde1/movies/disliked", async (c) => {
  try {
    ...
    const disliked = await kv.getByPrefix(
      `disliked:${user.id}:`,
    );
    ...
    return c.json(
      { error: `Failed to fetch disliked movies: ${error}` },
      500,
    );
  }
});
...
app.get("/make-server-5623fde1/movies/liked", async (c) => {
  try {
    ...
    const liked = await getByPrefixPaginated(
      `liked:${user.id}:`,
    );
    ...
    const moviesWithImdb = await Promise.all(
      liked.map(async (movie: any) => {
        const imdbRating = await kv.get(
          `imdb_rating:${movie.id}`,
        );
        return {
          ...movie,
    ...
    return c.json(
      { error: `Failed to fetch liked movies: ${error}` },
      500,
    );
  }
});
...
app.get(
  "/make-server-5623fde1/movies/partner-liked",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      const partnerLiked = await getByPrefixPaginated(
        `liked:${userProfile.partnerId}:`,
      );
      ...
      console.error(
        "Error fetching partner liked movies:",
        error,
      );
      return c.json(
        {
          error: `Failed to fetch partner liked movies: ${error}`,
        },
        500,
      );
    }
...
app.get("/make-server-5623fde1/movies/matches", async (c) => {
  try {
    ...
    const matches = await getByPrefixPaginated(
      `match:${user.id}:`,
    );
    ...
    return c.json(
      { error: `Failed to fetch matched movies: ${error}` },
      500,
    );
  }
});
  ...
  try {
    ...
    const settled = await pMap(
      ...
      async (movie: any) => {
        ...
        // Store liked movie
        await kv.set(
          `liked:${user.id}:${tmdbMovie.id}`,
          tmdbMovie,
        );

        // Check for partner match
        if (partnerId) {
          const partnerLiked = await kv.get(
            `liked:${partnerId}:${tmdbMovie.id}`,
          );
          if (partnerLiked) {
            await kv.set(
              `match:${user.id}:${tmdbMovie.id}`,
              tmdbMovie,
            );
            await kv.set(
              `match:${partnerId}:${tmdbMovie.id}`,
              tmdbMovie,
            );
          }
        }
    ...
    return c.json(
      { error: `Failed to import movies: ${error}` },
      500,
    );
  }
});
...
app.post("/make-server-5623fde1/movies/watched", async (c) => {
  try {
    ...
    if (!movie || !movie.id) {
      console.error(
        "Invalid movie data for watched - missing id:",
        movie,
      );
    ...
    if (!numericId || isNaN(numericId)) {
      console.error(
        `Invalid movie id (not coercible to number): ${movie.id} (type: ${typeof movie.id})`,
      );
      return c.json(
    ...
    console.log(
      `Stored watched movie ${numericId} ("${movie.title}") for user ${user.id} [id type: ${typeof numericId}]`,
    );
    return c.json({ success: true });
  } catch (error) {
    console.error("Error adding watched movie:", error);
    return c.json(
      { error: `Failed to add watched movie: ${error}` },
      500,
    );
  }
});
...
app.get("/make-server-5623fde1/movies/watched", async (c) => {
  try {
    ...
    const watched = await getByPrefixPaginated(
      `watched:${user.id}:`,
    );
    console.log(
      `Fetched ${watched.length} watched movies for user ${user.id}`,
    );
    return c.json({ movies: watched });
  } catch (error) {
    console.error("Error fetching watched movies:", error);
    return c.json(
      { error: `Failed to fetch watched movies: ${error}` },
      500,
    );
  }
});
...
app.delete(
  "/make-server-5623fde1/movies/watched/:movieId",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      return c.json(
        { error: `Failed to remove watched movie: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/movies/import-watched",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      for (let i = 0; i < settled.length; i++) {
        ...
        if (result.status === "fulfilled") {
          ...
          if (msg.startsWith("NOT_FOUND:")) {
            results.errors.push(
              `Movie not found: ${msg.replace("NOT_FOUND:", "")}`,
            );
          } else {
            results.errors.push(
              `Error importing ${movies[i].title}: ${result.reason}`,
            );
          }
        }
      }
      ...
      return c.json(
        { error: `Failed to import watched movies: ${error}` },
        500,
      );
    }
  },
...
app.get("/make-server-5623fde1/movies/discover", async (c) => {
  try {
    ...
    return c.json(
      { error: `Failed to discover movies: ${error}` },
      500,
    );
  }
});
...
app.get("/make-server-5623fde1/movies/search", async (c) => {
  try {
    ...
    return c.json(
      { error: `Failed to search movies: ${error}` },
      500,
    );
  }
});
...
app.get(
  "/make-server-5623fde1/movies/:id/details",
  async (c) => {
    try {
      const apiKey = Deno.env.get("TMDB_API_KEY");
      if (!apiKey) {
...
app.get("/make-server-5623fde1/search/people", async (c) => {
  try {
    ...
    return c.json(
      { error: `Failed to search people: ${error}` },
      500,
    );
  }
});
...
app.get("/make-server-5623fde1/search/keywords", async (c) => {
  try {
    ...
    return c.json(
      { error: `Failed to search keywords: ${error}` },
      500,
    );
  }
});
...
app.get(
  "/make-server-5623fde1/movies/:movieId/imdb",
  async (c) => {
    try {
      const movieId = c.req.param("movieId");
      ...
      if (
        cached &&
        cached.timestamp &&
        Date.now() - cached.timestamp < ONE_MONTH
      ) {
        // Return cached rating if less than 30 days old
        return c.json({
          imdbRating: cached.rating,
          cached: true,
        });
      }
      ...
      if (!tmdbData.imdb_id) {
        ...
        return c.json(
          { error: "IMDb ID not found for this movie" },
          404,
        );
      }
      ...
      if (!omdbResponse.ok) {
        console.error(
          `OMDb API HTTP error: ${omdbResponse.status}`,
        );
        return c.json(
      ...
      const contentType =
        omdbResponse.headers.get("content-type");
      if (
        !contentType ||
        ...
        console.error(
          `OMDb API returned non-JSON response: ${text.substring(0, 200)}`,
        );
        return c.json(
      ...
      if (omdbData.Response === "False") {
        console.log(
          `OMDb error for ${tmdbData.imdb_id}: ${omdbData.Error}`,
        );
        return c.json(
          {
            error:
              omdbData.Error || "IMDb rating not available",
          },
          404,
        );
      }
      ...
      if (
        ...
        console.log(
          `No IMDb rating available for movie ${movieId}`,
        );
        return c.json(
          { error: "IMDb rating not available" },
          404,
        );
      }
      ...
      console.error(
        `Error fetching IMDb rating for movie ${c.req.param("movieId")}:`,
        error,
      );
      return c.json(
        { error: `Failed to fetch IMDb rating: ${error}` },
        500,
      );
    }
  },
...
app.get(
  "/make-server-5623fde1/omdb/rating/:imdbId",
  async (c) => {
    try {
      ...
      if (omdbData.Response === "False") {
        console.log(
          `OMDb error for ${imdbId}: ${omdbData.Error}`,
        );
        return c.json(
          {
            error:
      ...
      console.error(
        `Error fetching IMDb rating for ${c.req.param("imdbId")}:`,
        error,
      );
      return c.json(
        { error: `Failed to fetch IMDb rating: ${error}` },
...
app.get(
  "/make-server-5623fde1/imdb-ratings/bulk",
  async (c) => {
    try {
      ...
      if (!tmdbIdsParam) {
        return c.json(
          { error: "tmdbIds parameter required" },
          400,
        );
      }
      ...
      for (const tmdbId of tmdbIds) {
        ...
        try {
          ...
          console.error(
            `Bulk fetch: failed to get key ${key}:`,
            keyError,
          );
        }
      }
      ...
      console.log(
        `Bulk fetch: ${ratings.length}/${tmdbIds.length} cached ratings found`,
      );
      return c.json(ratings);
    } catch (error) {
      console.error("Error in bulk fetch IMDb ratings:", error);
      return c.json(
        { error: `Failed to fetch cached ratings: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/imdb-ratings/store",
  async (c) => {
    try {
      const { tmdbId, imdbId, rating, votes } =
        await c.req.json();

      if (!tmdbId || !imdbId) {
        return c.json(
          { error: "tmdbId and imdbId are required" },
          400,
        );
      }
      ...
      console.log(
        `Stored IMDb rating for TMDb ${tmdbId}: ${rating}`,
      );
      ...
      return c.json(
        { error: `Failed to store rating: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/imdb-ratings/fetch-and-store",
  async (c) => {
    try {
      const { tmdbId, imdbId, releaseDate } =
        await c.req.json();
      ...
      if (
        errorCache &&
        errorCache.retryAfter &&
        Date.now() < errorCache.retryAfter
      ) {
        console.log(
          `Skipping ${tmdbId} - retry after ${new Date(errorCache.retryAfter).toISOString()}`,
        );
        return c.json(
          {
            error: "Recent error, retry later",
            retryAfter: errorCache.retryAfter,
          },
          429,
        );
      }
      ...
      if (cached?.fetchedAt) {
        ...
        if (
          (isRecentMovie && age < WEEK) ||
          (!isRecentMovie && age < MONTH)
        ) {
      ...
      if (omdbData.Response === "False") {
        console.log(
          `OMDb: no entry for ${imdbId}: ${omdbData.Error}`,
        );
      ...
      console.log(
        `Fetched and stored IMDb rating for TMDb ${tmdbId}: ${value.rating}`,
      );
      return c.json({ ...value, fromCache: false });
    } catch (error) {
      console.error(
        "Error fetching and storing IMDb rating:",
        error,
      );
      return c.json(
        { error: `Failed to fetch rating: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/movies/update-imdb-ratings",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      const likedMovies = await getByPrefixPaginated(
        `liked:${user.id}:`,
      );
      ...
      for (const movie of likedMovies) {
        try {
          ...
          console.error(
            `Error updating IMDb rating for movie ${movie.id}:`,
            error,
          );
          results.failed++;
        }
      ...
      return c.json(
        { error: `Failed to update IMDb ratings: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/movies/not-interested",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      console.error(
        "Error marking movie as not interested:",
        error,
      );
      return c.json(
        { error: `Failed to mark as not interested: ${error}` },
        500,
      );
    }
  },
...
app.delete(
  "/make-server-5623fde1/movies/not-interested/:movieId",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      console.error(
        "Error removing not interested status:",
        error,
      );
      return c.json(
        {
          error: `Failed to remove not interested status: ${error}`,
        },
        500,
      );
    }
...
app.get(
  "/make-server-5623fde1/movies/excluded-ids",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      return c.json(
        { error: `Failed to fetch excluded IDs: ${error}` },
        500,
      );
    }
  },
...
app.post(
  "/make-server-5623fde1/movies/interactions",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      return c.json(
        { error: `Failed to fetch interactions: ${error}` },
        500,
      );
    }
  },
...
app.get(
  "/make-server-5623fde1/movies/interactions/all",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      for (const { key, value } of watchedRaw) {
        ...
        // Old Letterboxd sync entries may only contain { rating, timestamp } with
        // no id field; the key suffix is always reliable.
        const keyId = Number(key.split(":").pop());
        const rawId = item.id ?? item.tmdbId ?? (keyId > 0 ? keyId : undefined);
        const tmdbId = Number(rawId);
        if (!tmdbId || isNaN(tmdbId)) {
          // Truly unrecoverable — key itself has no numeric suffix
          console.warn(
            `interactions/all: Skipping watched entry with unrecoverable id (key: ${key})`,
          );
          continue;
        }
        {
          const existing = interactions.find(
            (i: any) => i.tmdbId === tmdbId,
          );
          if (existing) {
            existing.isWatched = true;
            existing.watchedAt = item.timestamp
      ...
      for (const item of notInterestedItems) {
        ...
        if (!tmdbId || isNaN(tmdbId)) continue;
        {
          ...
          if (existing) {
            ...
            existing.notInterestedAt = item.timestamp
              ? new Date(item.timestamp).toISOString()
              : null;
          } else {
            interactions.push({
              ...
              watchedAt: null,
              notInterestedAt: item.timestamp
                ? new Date(item.timestamp).toISOString()
                : null,
            });
          }
      ...
      console.log(
        `interactions/all: Returning ${interactions.length} total interactions`,
      );
      return c.json({ interactions });
    } catch (error) {
      console.error(
        "Error fetching all user interactions:",
        error,
      );
      return c.json(
        { error: `Failed to fetch all interactions: ${error}` },
        500,
      );
    }
  },
...
app.get(
  "/make-server-5623fde1/movies/discover-filtered",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      if (!includeWatched) {
        ...
        excludedIds = [...excludedIds, ...watchedIds];
        console.log(
          `discover-filtered: Loaded ${watchedIds.length} watched ids to exclude for user ${user.id}`,
        );
      }

      const excludedSet = new Set(excludedIds);
      console.log(
        `Excluding ${excludedIds.length} movies for user ${user.id}`,
      );

      // Resolve director/actor names to TMDb person IDs
      let withCrew = "";
      let withCast = "";
      const streamingServices = c.req.query(
        "streamingServices",
      );
      ...
      while (allMovies.length < 20 && attempts < maxAttempts) {
        ...
        console.log(
          `Fetching TMDb page ${currentPage} (attempt ${attempts + 1})`,
        );
        ...
        if (data.results && Array.isArray(data.results)) {
          ...
          const validMovies = data.results.filter(
            (movie: any) => !excludedSet.has(movie.id),
          );

          console.log(
            `Page ${currentPage}: Found ${validMovies.length}/${data.results.length} valid movies`,
          );
      ...
      return c.json(
        { error: `Failed to fetch filtered movies: ${error}` },
        500,
      );
    }
  },
...
app.get(
  "/make-server-5623fde1/debug/watched/:movieId",
  async (c) => {
    try {
      const accessToken = c.req
        .header("Authorization")
      ...
      // Direct key lookup
      const directKey = await kv.get(
        `watched:${user.id}:${movieId}`,
      );

      // Prefix scan – look for any entry whose stored id matches (paginated for accuracy)
      const allWatched = await getByPrefixPaginated(
        `watched:${user.id}:`,
      );
